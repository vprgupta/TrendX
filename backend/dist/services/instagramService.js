...
const getInstagramTrends = async () => {
    try {
        ...
        const $ = await (0, scraperUtils_1.fetchHtml)('https://hashtagify.me/hashtag/trending');
        if (!$)
            return [];
        const trends = [];
        ...
        $('table tr').each((i, el) => {
            ...
            if (trends.length >= 10)
                return;
            const name = (0, scraperUtils_1.cleanText)($(el).find('td').eq(0).text());
            const posts = (0, scraperUtils_1.cleanText)($(el).find('td').eq(1).text());
